...
provide(bemDom.declBlock(this.name, {
    ...
    checkSessionTime: function() {
        ...
        if (sessionStorage.time && ((diffTime = (Math.floor((new Date - (new Date(sessionStorage.time))) / 60000))) < constTime)) {
            setTimeout(function() {
                _this.exit();
            }, (constTime - diffTime) * 60 * 1000);
        }
    },
    ...
    conrolTariffsFamilyAndMultinet: function() {
        ...
        if (ctx.getSession('authentication', 'access_token')) {
            ctx.checkProfile();
        } else {
            nextTick(function() {
    ...
    goToManagement: function() {
        ...
        if (ctx.getSession('authentication', 'access_token')) {
            if (this.getSession('profile', 'role') === 'ROLE_BUNDLE_CHIEF') {
    ...
    authEntrance: function(actionParams, _this) {
        ...
        if (!phoneNumber || !formAuth['password']) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data['code'] && data['code'] === 'SUCCESS') {
                        ...
                        ctx.getMessage('ERR-01', '.form__text-error');
                        form.findChildElem('text-error').setMod('hidden', 'on');
            ...
            function check_token_ajax() {
                $.ajax({
                    type: "POST",
                    url: "api/check_token",
                    data: { token: ctx.getSession('authentication', 'access_token') },
                    success: function(data) {
                        var data = JSON.parse(data);
                        console.log(data.data);

                        if (data && data.data.authorities) {
                            sessionStorage.setItem('authorities', JSON.stringify(data));
                            get_profile_ajax();
                        } else {
                            ctx.afterAuth(actionParams);
                        }
                    }
                });
            }

            function get_profile_ajax() {
                $.ajax({
                    type: "POST",
                    url: "api/get_profile",
                    data: {
                        access_token: ctx.getSession('authentication', 'access_token')
                    },
                    success: function(data) {
                        console.log(data);
    ...
    afterAuth: function(actionParams) {
        var ctx = this;
        ...
        if (actionParams['typeTariff']) {
            ...
            var familyTariff = { 'SMP': true, 'SMP1': true, 'SMP2': true, 'SMP3': true, 'SMPN': true },
                authorities = ctx.getSession('authorities', 'authorities'),
                tariffCode = this.getSession('profile', 'tariff')['code'];

            if (!authorities || (authorities && tariffCode && !familyTariff[tariffCode])) {
                $('.modal__close-button').click();
            } else {
                if (authorities[0] === 'ROLE_BUNDLE_CHIEF') {
                    window.location = "http://multidev.life.com.by/main";
                } else if (authorities[0] === 'ROLE_BUNDLE_USER') {
                    window.location = "http://multidev.life.com.by/user";
                }
            }
        }
    ...
    checkProfile: function() {
        ...
        if (tariffCode && familyTariff[tariffCode]) {
            console.log('family');
            if (this.getSession('profile', 'role') === 'ROLE_BUNDLE_CHIEF') {
                window.location = "http://multidev.life.com.by/main";
            } else {
    ...
    confirmTariffChange: function(actionParams) {
        ...
        $.ajax({
            ...
            data: {
                'msisdn': _this.getSession('profile', 'msisdn'),
                'tariff': actionParams['typeTariff'],
                'access_token': _this.getSession('authentication', 'access_token')
            },
            success: function(data) {
                console.log(data);
    ...
    getSession: function(sessionKey, el) {
        if (sessionStorage[sessionKey]) {
            if (el) {
                return JSON.parse(sessionStorage[sessionKey]).data[el];
            } else {
                return JSON.parse(sessionStorage[sessionKey]);
            }
        }
    ...
    switchButtons: function() {
        var familyTariff = { 'SMP': true, 'SMP1': true, 'SMP2': true, 'SMP3': true, 'SMPN': true },
            authorities = this.getSession('authorities', 'authorities'),
            tariffCode = this.getSession('profile', 'tariff')['code'],
            access_token = this.getSession('authentication', 'access_token');

        $('.button-exit')[(this.getSession('authentication', 'access_token') ? 'remove' : 'add') + 'Class']('hide');

        console.log(authorities);
        console.log(tariffCode);
        console.log(access_token);

        if (!access_token || (access_token && tariffCode && familyTariff[tariffCode])) {
            $('.button-line, .conrolTariffsFamilyAndMultinet, .sep-in-menu').removeClass('hide');
        } else {
            $('.button-line, .conrolTariffsFamilyAndMultinet, .sep-in-menu').addClass('hide');
        }
    },
